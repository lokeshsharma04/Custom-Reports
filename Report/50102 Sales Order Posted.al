report 50102 "Sales Order Posted"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layout/Sales Order Posted.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                column(CGSTAmt; Abs(CGSTAmt))//"GST Component".Description)
                {
                }
                column(CessPer; CessPer)//"GST Component".Description)
                {
                }
                column(CessAmt; ABS(CessAmt))//"GST Component".Description)
                {
                }
                column(IGSTAmnt; ABS(IGSTAmnt))//"GST Component".Description)
                {
                }
                ...
                begin
                    ...
                    // repcheck.FormatNoText(Notext2, TotalGST, "Sales Invoice Header"."Currency Code");
                    // AmountInWord2 := Notext2[1] + Notext2[2];
                    // Message('%1', AmountInWord2);
