report 50149 CheckCOpy
{
    ...
    begin
        ...
        if currency.get(CurrencyCode) then
            AddToNoText(NoText, NoTextIndex, PrintExponent, currency.Description + '' + Text028)
        else
            AddToNoText(NoText, NoTextIndex, PrintExponent, 'Rupee And');
        //NEW IG_KS
        // old code 
        ...
        IF (CurrencyCode <> '') THEN
            AddToNoText(NoText, NoTextIndex, PrintExponent, ' ' + currency.Description + 'Cents ONLY')
        ELSE
        ...
        /*    DecimalPosition := GetAmtDecimalPosition;
            ...
            if (No * DecimalPosition) <> 0 then// IG_KS
                FractionPart := Round((No * DecimalPosition) / (DecimalPosition), 0.01)
            else
                FractionPart := 0;



            //   AddToNoText(NoText, NoTextIndex, PrintExponent, (Format(No * DecimalPosition) + '/' + Format(DecimalPosition))); ig_KS

            AddToNoText(NoText, NoTextIndex, PrintExponent, Format(FractionPart));
            if CurrencyCode <> '' then
                AddToNoText(NoText, NoTextIndex, PrintExponent, CurrencyCode)
            else
                AddToNoText(NoText, NoTextIndex, PrintExponent, Format(FractionPart) + 'PAISA ONLY');
    */
